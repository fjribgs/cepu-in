...
export default function DashboardSidebar() {
    return (
        <aside className="relative flex flex-col w-auto min-w-55 h-screen bg-(--color-light) justify-between items-center py-6">
            <div className="flex flex-col gap-2.5 w-full px-6">
                <div className="flex w-full justify-center">
                    <Image src="/logo.svg" width={150} height={150} alt="Logo" className=""/>
                </div>
                
                <div id="separator" className="h-0.5 w-full bg-black/20"></div>

                {SidebarLinks.map((item) => (
                    <Link href={item.href} key={item.id} className="flex h-full items-center gap-3 font-nunito font-semibold text-(--color-normal) hover:bg-(--color-light-hover) hover:px-2 py-1.5 rounded-[7] transition-all duration-200 ease-in cursor-pointer">
